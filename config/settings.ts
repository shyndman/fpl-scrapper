...
function readFloat(env: NodeJS.ProcessEnv, key: string, fallback: string): number {
  const rawValue = env[key] ?? fallback;

  if (rawValue.trim() === '') {
    throw new Error(`Invalid numeric value for ${key}: ${JSON.stringify(rawValue)}`);
  }
...
function readInt(env: NodeJS.ProcessEnv, key: string, fallback: string): number {
  const rawValue = env[key] ?? fallback;
