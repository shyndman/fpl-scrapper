...
describe("config/settings.ts", () => {
  ...
  it("falls back to project-root defaults", async () => {
    ...
    expect(settings.LOG_FILE).toBe(
      resolve(PROJECT_ROOT, "logs", "fpl_scraper.log"),
    );
    expect(settings.SESSION_FILE).toBe(
      resolve(PROJECT_ROOT, "data", ".session.json"),
    );
  ...
  it("throws on invalid numeric values", async () => {
    ...
    expect(() =>
      loadSettings({ ...process.env, REQUEST_DELAY_MIN: "" }),
    ).toThrow("Invalid numeric value for REQUEST_DELAY_MIN");
  });
});
