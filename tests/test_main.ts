...
import { type CliDeps, EXIT_FATAL, EXIT_PARTIAL, EXIT_SUCCESS, buildParser, main } from '../src/main.ts';
...
function createSyncResult(errors: number, playersSynced: number, summary: string): SyncResultStub {
  return {
    errors,
    playersSynced,
    summary: () => summary,
...
function createDeps(): {
  ...
  const api = {
    discover: vi.fn(async () => ({ 'bootstrap-static': { keys: ['elements'], type: 'dict' } })),
  };
  const syncer = {
    fullSync: vi.fn(async () => createSyncResult(0, 3, 'mode=full_sync players=3 requests=12 errors=0')),
    gameweekSync: vi.fn(async () =>
      createSyncResult(0, 1, 'mode=gameweek_sync gw=7 players=1 requests=2 errors=0'),
    ),
  };
...
describe('src/main.ts buildParser', () => {
  it('requires exactly one mode and parses overrides', () => {
    ...
    expect(() => parser.parseArgs([])).toThrowError(/Exactly one mode is required/u);
    expect(() => parser.parseArgs(['--full-sync', '--current-gameweek'])).toThrowError(
...
describe('src/main.ts main', () => {
  ...
  it('dispatches current and explicit gameweek sync modes with stable exit codes', async () => {
    ...
    current.syncer.gameweekSync.mockResolvedValueOnce(
      createSyncResult(2, 5, 'mode=gameweek_sync gw=25 players=5 requests=9 errors=2'),
    );

    await expect(main(['--current-gameweek'], current.deps)).resolves.toBe(EXIT_PARTIAL);
    expect(current.syncer.gameweekSync).toHaveBeenCalledWith();
    expect(current.db.closed).toBe(true);

    const explicit = createDeps();
    await expect(main(['--gameweek', '7'], explicit.deps)).resolves.toBe(EXIT_SUCCESS);
  ...
  it('treats sync errors with no scraped players as fatal', async () => {
    ...
    deps.createSyncer = () => ({
      fullSync: vi.fn(async () => createSyncResult(1, 0, 'mode=full_sync players=0 requests=4 errors=1')),
      gameweekSync: vi.fn(),
    });
  ...
  it('prints discovery JSON, skips sync dispatch, and closes the database', async () => {
    ...
    expect(JSON.parse(output[0] ?? 'null')).toEqual({ 'bootstrap-static': ['elements'] });
