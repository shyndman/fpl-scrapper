...
type ScraperGet = (
  path: string,
  options?: { requiresAuth?: boolean; params?: Record<string, unknown> },
) => Promise<unknown>;
...
describe("src/api.ts", () => {
  ...
  it.each([
    ...
    "forwards auth-required endpoint %s",
    async (methodName, args, expectedPath) => {
      const scraper = createScraperStub();
      scraper.get.mockResolvedValue({ ok: true });
      const api = new FPLAPI(scraper);

      const result = await (
        api[methodName] as (...callArgs: number[]) => Promise<unknown>
      )(...args);

      expect(result).toEqual({ ok: true });
      expect(scraper.get).toHaveBeenCalledOnce();
      expect(scraper.get).toHaveBeenCalledWith(expectedPath, {
        requiresAuth: true,
      });
    },
  );
  ...
  it("reports discovery shapes and uses the current gameweek from a second bootstrap probe", async () => {
    ...
    vi.spyOn(api, "getFixtures").mockImplementation(
      async (gameweek?: number) => {
        expect(gameweek).toBeUndefined();
        return [{ id: 10, team_h: 1 }];
      },
    );
    vi.spyOn(api, "getElementSummary").mockImplementation(
      async (playerId: number) => {
        expect(playerId).toBe(1);
        return { fixtures: [], history: [] };
      },
    );
    vi.spyOn(api, "getEventLive").mockImplementation(
      async (gameweek: number) => {
  ...
  it("records probe failures without aborting discovery", async () => {
    ...
    vi.spyOn(api, "getFixtures").mockRejectedValue(
      new Error("fixtures failed"),
    );
    vi.spyOn(api, "getElementSummary").mockRejectedValue(
