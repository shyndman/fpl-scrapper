...
function createScraper(
  ...
  const fetchMock = createFetchStub(
    ...(options.fetchResults ?? [createJsonResponse(200, {})]),
  );
  const sleepMock =
    options.sleep ??
    vi.fn<(milliseconds: number) => Promise<void>>(async () => undefined);

  const scraper = new FPLScraper(
    auth,
...
describe("src/scraper.ts", () => {
  ...
  it("returns JSON for a successful 200 response", async () => {
    ...
    await expect(scraper.get("bootstrap-static")).resolves.toEqual({
      elements: [],
    });
  ...
  it("raises FPLAPIError after exhausting retries on 5xx responses", async () => {
    const { scraper } = createScraper({
      fetchResults: Array.from({ length: 5 }, () =>
        createJsonResponse(503, { error: "down" }),
      ),
      maxRetries: 5,
    });
  ...
  it("re-authenticates once on 403 and retries the request", async () => {
    ...
    const { scraper } = createScraper({
      ...
      fetchResults: [
        ...
        createJsonResponse(200, { picks: [] }),
      ],
    });

    await expect(
      scraper.get("my-team/1", { requiresAuth: true }),
    ).resolves.toEqual({
      picks: [],
    });
  ...
  it("honors Retry-After on 429 responses and surfaces FPLRateLimitError after the final retry", async () => {
    ...
    await expect(scraper.get("fixtures")).rejects.toThrowError(
      FPLRateLimitError,
    );
  ...
  it("injects auth cookies into authenticated requests", async () => {
    const auth = createAuthStub({
      sessionid: "cookie-123",
      pl_profile: "profile-456",
    });
    const { fetchMock, scraper } = createScraper({
      auth,
    ...
    expect(headers.get("Cookie")).toBe(
      "sessionid=cookie-123; pl_profile=profile-456",
    );
