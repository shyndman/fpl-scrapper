...
function countTransports(logger: ReturnType<typeof setupLogging>) {
  return {
    console: logger.transports.filter((transport) => transport instanceof winstonTransports.Console).length,
    file: logger.transports.filter((transport) => transport instanceof winstonTransports.File).length,
  };
}

async function waitForLogLine(logFile: string, expectedText: string): Promise<string> {
...
describe('src/logger.ts', () => {
  ...
  it('reinitialization replaces previous transports without duplicates', async () => {
    ...
    expect(firstTransports.every((transport) => !secondLogger.transports.includes(transport))).toBe(true);
